...
func syncCopy(args []string, recursive bool, include string, exclude string, meta util.Meta, storageClass string) {
    ...
    logger.Infof("bucketName1: %s", bucketName1)
    logger.Infof("bucketName2: %s", bucketName2)
    logger.Infof("cosPath1: %s", cosPath1)
    logger.Infof("cosPath2: %s", cosPath2)
    ...
    if recursive {
        ...
        logger.Infof("objects is %v", objects)
        ...
        for _, o := range objects {
            ...
            logger.Infof("dstKey is %s", dstKey)
            ...
            logger.Infof("request is %v, err is %v", *resp, err)
            ...
            if err != nil {
                if resp != nil && resp.StatusCode == 404 {
                    ...
                    logger.Infoln("srcUrl is %s", srcURL)
        ...
        if err != nil {
            if resp != nil && resp.StatusCode == 404 {
                ...
                logger.Infof("url.Host: %s", url.BucketURL.Host)
                srcURL := fmt.Sprintf("%s/%s", url.BucketURL.Host, cosPath1)
                logger.Infof("srcURl:%s", srcURL)
