...
func removeObjects1(args []string, include string, exclude string, force bool) {
    ...
    for _, arg := range args {
        ...
        // 确认存储桶是否为融合桶
        responseBucket, errBucket := c.Bucket.Head(context.Background(), &cos.BucketHeadOptions{})
        if errBucket != nil {
            logger.Fatalln(errBucket)
            os.Exit(1)
        }
        flagOfs := responseBucket.Header.Get("x-cos-bucket-arch")
        ...
        for isTruncated {
            if flagOfs == "OFS" {
                ...
                //}
                recursiveRemoveObject1(bucketName, cosDir, force)
                isTruncated = false
            } else {
                objects, t, m, _ := util.GetObjectsListIterator(c, cosDir, nextMarker, include, exclude)
                isTruncated = t
                nextMarker = m
...
func recursiveRemoveObject1(bucketName string, cosPath string, force bool) {
    c := util.NewClient(&config, &param, bucketName)
    opt := &cos.ObjectDeleteOptions{
        XCosSSECustomerAglo:   "",
        XCosSSECustomerKey:    "",
